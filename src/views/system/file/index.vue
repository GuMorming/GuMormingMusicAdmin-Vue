    ...
    <el-table :data="fileList" @selection-change="handleSelectionChange" v-loading="loading">
      ...
      <el-table-column prop="isDir" width="56">
        <template #default="scope">
          ...
          <el-image v-else-if="scope.row.fileUrl"
                    lazy
                    class="article-cover"
                    :src="scope.row.fileUrl"></el-image>
      ...
      <el-table-column prop="fileName" sortable label="文件名">
        <template #default="scope">
          <span style="cursor:pointer;"
                @click="handleOpen(scope.row)">{{ scope.row.fileName }}</span>
        </template>
      </el-table-column>
      <!-- 类型 -->
      ...
      <el-table-column label="操作" align="center" width="150">
        <template #default="scope">
          <el-button type="primary" icon="View" link>
            <template #default>
              <a :href="`/api/admin/file/download/${scope.row.id}`">下载</a>
            </template>
          </el-button>
...
import {File, FileQuery, FolderForm} from '@/api/file/types';
...
const getFileType = computed(() => (file: File) => {
